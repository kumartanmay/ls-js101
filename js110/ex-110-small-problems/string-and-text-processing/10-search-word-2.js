 ...
 **SED** quis autem vel est, iure reprehenderit qui in ea voluptate velit esse quam nihil molestiae consequatur. 
...
function searchWord(word, text) {
  ...
  // find indices of the search word
  let searchIndices = [];
  textArr.forEach((elem, index) => {
    if(elem.toLowerCase() === word) searchIndices.push(index);
  })

  for (let i = 0; i < searchIndices.length; i++) {
    textArr[searchIndices[i]] = '**' + textArr[searchIndices[i]].toUpperCase() + '**';
  }
